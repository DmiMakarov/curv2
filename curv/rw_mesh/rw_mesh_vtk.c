...
int _read_vtk_normals_type(FILE*fd,int count,double*value,int*current_line){
	int i=0;
	char line[256];
	int rn;
	line[0]=0;
	while(i<count){
		...
		rn = sscanf(line,"%lf%lf%lf", value+3*i, value + 3*i+1, value + 3*i+2 );
		if(rn!=3){
			rw_mesh_set_error(*current_line,"Error in reading normals values");
			return 1;
		}
...
int read_format_vtk_struct(struct RW_MESH_VTK_STRUCT*VTK, const char *filename,int flags){
		...
		{// Cells block
			...
			{// Cell types block
				...
				if(isBinary){
					...
					if(_read_vtk_array_type_ascii(fd,UnstructuredGrid->CountOfCells,RW_MESH_VTK_TYPE_INT,UnstructuredGrid->CellTypes,&current_line)){
						rw_mesh_set_error(current_line,"Can't read count of cell type");
						return 1;
					}
					/*
	...
	while(read_line_skip_empty(fd,line,&current_line)>=0){
		...
		if(strcmp(word,"POINT_DATA")==0){
		...
		}else if(strcmp(word,"NORMALS")==0){
			// first word SCALARS
			if(isBinary){
				printf("Binary with normals isn't supported in current version of reader\n");
				return 1;
			}
			struct RW_MESH_VTK_DATASET_STRUCT*Data;
			struct RW_MESH_VTK_DATA_NORMALS_STRUCT*Normals;
			Data = _vtk_add_data(VTK,data_object);
			Data->DataType = RW_MESH_VTK_DATASET_TYPE_NORMALS;
			Data->Counts = data_size;
			Normals = (struct RW_MESH_VTK_DATA_NORMALS_STRUCT*)calloc(Data->Counts,sizeof(struct RW_MESH_VTK_DATA_NORMALS_STRUCT));
			Data->Data = Normals;
			...
			//next word must be type
			string_cut_word(line,word);
			Normals->dataType = _parse_vtk_type(word);

			if(Normals->dataType == RW_MESH_VTK_TYPE_NONE){
				rw_mesh_set_error(current_line,"Unknown type");
				return 1;
			}
			ftell_position = ftell(fd);
			...
			if(strcmp(word,"LOOKUP_TABLE")==0){
				...
				string_cut_word(line,word);
				string_cut_word(line,word);
				Normals->LookupTable = (char*)calloc(strlen(word)+1,sizeof(char));
				strcpy(Normals->LookupTable,word);
			}else{
				// no, come back and read data
				current_line--;
				fseek(fd,ftell_position,SEEK_SET);
			}
			Normals->values = (double *) malloc(sizeof(double *) * Data->Counts*3);
			if(Normals->values == NULL){ printf("Can't allocate memory fo normals\n"); return 1; }
			if(_read_vtk_normals_type(fd,Data->Counts,Normals->values,&current_line)){
				return 1;
			}
		}else{
...
int _write_data(FILE*fd,struct RW_MESH_VTK_DATASET_STRUCT*Data,int isBinary){
	...
	case RW_MESH_VTK_DATASET_TYPE_NORMALS:
		...
		Normals = (struct RW_MESH_VTK_DATA_NORMALS_STRUCT*)Data->Data;
		...
		//dataType
		_get_data_type_name(Normals->dataType,string);
		fprintf(fd," %s\n",string);

		if(isBinary){
			_write_array_data_type_binary(fd,Normals->values,Data->Counts*3,Normals->dataType);
		}else{
			_write_array_data_type(fd,Normals->values,Data->Counts*3,Normals->dataType,3);
		}
		break;
	case RW_MESH_VTK_DATASET_TYPE_TEXTURE_COORDINATES:
...
int write_format_vtk_struct(struct RW_MESH_VTK_STRUCT*VTK,const char *filename,int flags){
	...
	fprintf(fd,"File generated by rw_mesh\n");
	if(isBinary){
		fprintf(fd,"BINARY\n");
	}else{
		fprintf(fd,"ASCII\n");
...
void vtk_data_field_add_array(struct RW_MESH_VTK_DATA_FIELD_STRUCT*Field,const char*name,int Count,int numComp,int type,void*Values){
	int i,j;
...
void vtk_data_field_add_array_mask(struct RW_MESH_VTK_DATA_FIELD_STRUCT*Field,const char*name,int Count,int numComp,int type,void*Values,int*mask){
	int i,j;
	struct RW_MESH_VTK_DATA_FIELD_ARRAY_STRUCT* Array;
	...
	for(i=j=0;i<Count;i++)
		if(mask[i]){
			memcpy((char*)(Array->values)+j*Array->numComponents*_vtk_type_size(type),(char*)Values+i*Array->numComponents*_vtk_type_size(type),Array->numComponents*_vtk_type_size(type));
			j++;
		}
...
struct RW_MESH_VTK_STRUCT* rw_mesh_vtk_create_unstructured_simplified_uniform(int CountOfPoints, REAL*Points, int dim, int CountOfCells,int*Cells,int CellType){
	struct RW_MESH_VTK_STRUCT*VTK;
	int i,s;
	int*CellSizes;
	int*CellTypes;
	...
	for(i=0;i<CountOfCells;i++){
		...
		switch(CellType){
			case RW_MESH_VTK_CELL_TYPE_NONE: s = 0; break;
			case RW_MESH_VTK_CELL_TYPE_TRIANGLE: s = 3; break;
			case RW_MESH_VTK_CELL_TYPE_QUAD: s = 4; break;
			case RW_MESH_VTK_CELL_TYPE_TETRA: s = 4; break;
			case RW_MESH_VTK_CELL_TYPE_VOXEL: s = 8; break;
			case RW_MESH_VTK_CELL_TYPE_HEXAHEDRON: s = 8; break;
			case RW_MESH_VTK_CELL_TYPE_WEDGE: s = 6; break;
			case RW_MESH_VTK_CELL_TYPE_PYRAMID: s = 5; break;
			default: return NULL;
		}
		CellOffset[i+1] = CellOffset[i] + s;
		CellSizes[i] = s;
	}

	if (dim == 3){
		p = (REAL3*)Points;
		VTK = rw_mesh_vtk_create_unstructured_simplified(CountOfPoints, p, CountOfCells, Cells, CellSizes, CellTypes, CellOffset);
		free(CellSizes);
		free(CellTypes);
		free(CellOffset);
		return VTK;
	...
	{
		p = (REAL3*)malloc(CountOfPoints * sizeof(REAL3));
		for (i = 0; i < CountOfPoints; i++)
		{
			p[i][0] = Points[2 * i + 0];
			p[i][1] = Points[2 * i + 1];
...
int rw_mesh_vtk_add_scalars(struct RW_MESH_VTK_STRUCT*VTK,int data_object,int Count,int data_type,void*values,const char*name){
	struct RW_MESH_VTK_DATASET_STRUCT*Data;
	struct RW_MESH_VTK_DATA_SCALARS_STRUCT*Scalars;
	Data = _vtk_add_data(VTK,data_object);
...
int rw_mesh_vtk_add_mask(struct RW_MESH_VTK_STRUCT*VTK,int data_object,int Count,int*Mask,const char*name){
	return rw_mesh_vtk_add_scalars(VTK,data_object,Count,RW_MESH_VTK_TYPE_INT,Mask,name);
}

int rw_mesh_vtk_add_mask_points(struct RW_MESH_VTK_STRUCT*VTK,int Count,int*Mask,const char*name){
	return rw_mesh_vtk_add_mask(VTK,RW_MESH_VTK_DATA_OBJECT_POINTS,Count,Mask,name);
}
int rw_mesh_vtk_add_mask_cells(struct RW_MESH_VTK_STRUCT*VTK,int Count,int*Mask,const char*name){
	return rw_mesh_vtk_add_mask(VTK,RW_MESH_VTK_DATA_OBJECT_CELLS,Count,Mask,name);
}

int rw_mesh_vtk_add_function(struct RW_MESH_VTK_STRUCT*VTK,int data_object,int Count,REAL*Function,const char*name){
	return rw_mesh_vtk_add_scalars(VTK,data_object,Count,RW_MESH_VTK_TYPE_DOUBLE,Function,name);
}
int rw_mesh_vtk_add_function_points(struct RW_MESH_VTK_STRUCT*VTK,int Count,REAL*Function,const char*name){
	return rw_mesh_vtk_add_function(VTK,RW_MESH_VTK_DATA_OBJECT_POINTS,Count,Function,name);
}
int rw_mesh_vtk_add_function_cells(struct RW_MESH_VTK_STRUCT*VTK,int Count,REAL*Function,const char*name){
	return rw_mesh_vtk_add_function(VTK,RW_MESH_VTK_DATA_OBJECT_CELLS,Count,Function,name);
}

int rw_mesh_vtk_add_vectors(struct RW_MESH_VTK_STRUCT*VTK,int data_object,int Count,int data_type,void*values,const char*name){
	struct RW_MESH_VTK_DATASET_STRUCT*Data;
...
int rw_mesh_vtk_add_vectors_mask(struct RW_MESH_VTK_STRUCT*VTK,int data_object,int Count,int data_type,void*values,int*mask,const char*name){
	int i,j;
	struct RW_MESH_VTK_DATASET_STRUCT*Data;
	struct RW_MESH_VTK_DATA_VECTORS_STRUCT*Vectors;
...
struct RW_MESH_VTK_DATA_FIELD_STRUCT* vtk_data_field_add_field(struct RW_MESH_VTK_STRUCT*VTK,int data_object,const char*name){
